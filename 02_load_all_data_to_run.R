...
load("fit_sum_core.Rdata")
fit_sum_core <- fit.sum
...
f_age_foi <- fit_sum_core[grep("f_age",
                         rownames(fit_sum_core)), 1][1:n_ageclassf] -
             fit_sum_core[grep("f_age_mu",rownames(fit_sum_core)),1] +
             fit_sum[grep("f_age_foi_mu",rownames(fit_sum)), 1]

m_age_foi <- fit_sum_core[grep("m_age",
                         rownames(fit_sum_core)), 1][1:n_ageclassm] -
             fit_sum_core[grep("m_age_mu",rownames(fit_sum_core)),1] +
             fit_sum[grep("m_age_foi_mu",rownames(fit_sum)), 1]
